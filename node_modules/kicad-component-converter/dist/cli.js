...
// src/cli/cmds/convert-kicad-directory-to-ts.ts
import {
  readFileSync,
  readdirSync,
  writeFileSync,
...
function normalizeFileNameToVarName(str) {
  return str.replace(/[^a-zA-Z0-9]/g, "_").replace(/-([a-z])/g, (g) => g[1].toUpperCase());
}
async function getTsFileContentFromKicadModFile(kicadModFilePath) {
...
var convertKicadDirectoryToTs = async (args) => {
  ...
  if (!existsSync(join(outputDir, "package.json"))) {
    ...
    const tscircuitPackageInfo = await fetch(
      "https://registry.npmjs.org/tscircuit"
    ).then((r) => r.json());
    const typescriptPackageInfo = await fetch(
    ...
    if (!existsSync(join(outputDir, ".github/workflows/tsci-publish.yml"))) {
      const { shouldCreateGithubWorkflow } = await prompts({
        ...
        type: "confirm",
        message: "BETA FEATURE: Create a GitHub workflow for CI/CD to autopublish to the tscircuit registry?"
      });
...
`.trim()
  ...
  const kicadModFiles = scanDirectory(inputDir).map(
    (fpath) => relative(inputDir, fpath)
  );
...
`;
      ...
      return {
        varName,
        relativePath: `./${relative(outputDir, outputFilePath).replace(/\\/g, "/").replace(/\.tsx$/, "")}`
      };
    })
  );
  const indexContent = exports.map(
    (exp) => `export { default as ${exp.varName} } from '${exp.relativePath}';`
  ).join("\n");
...
// src/cli/param-handlers/interact-for-local-directory.ts
...
// src/cli/param-handlers/index.ts
...
program.command("convert-kicad-directory").requiredOption("--input-dir <input>", "Input directory").requiredOption("--output-dir <output>", "Output directory").action((opts) => convertKicadDirectoryToTs(opts));
perfectCli(program, process.argv, {
  async customParamHandler({ commandPath, optionName }, { prompts: prompts2 }) {
    const optionNameHandler = PARAM_HANDLERS_BY_PARAM_NAME[optionName.replace(/-/g, "_")];
